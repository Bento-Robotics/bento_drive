...
namespace edu
{
    ...
    void EduDrive::rpmCallback(const std_msgs::msg::Float32MultiArray::SharedPtr rpm)
    {
        ...
        for (unsigned int i = 0; i < _mc.size(); i++)
        {
            ...
            if (std::isnormal(rpmVals[0]) != std::isnormal(rpmVals[1]))
            {
                RCLCPP_WARN_STREAM(this->get_logger(), "#EduDrive RPM override data invalid. Overriding individual MotorController channels not possible. Skipping controller " << i);
                continue;
            }

            if (std::isnan(rpmVals[0]) || std::isnan(rpmVals[1]))
                continue;
    ...
    void EduDrive::controlMotors(float vFwd, float vLeft, float omega)
    {
        ...
        for (unsigned int i = 0; i < _mc.size(); ++i)
        {
            ...
            if (this->get_parameter( std::string("controller") + std::to_string(i) + std::string(".doKinematics") ).as_bool() == false) continue;
